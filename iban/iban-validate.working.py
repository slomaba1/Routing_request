...
def _get_iban_spec(country_code):
    try:
        ...
        raise exceptions.InvalidCountryCode("Unknown country-code '{country_code}'")
...
class IBAN(common.Base):
    ...
    def generate(cls, country_code, bank_code, account_code, branch_code=""):
        ...
        if len(bank_code) > bank_code_length:
            raise exceptions.InvalidBankCode("Bank code exceeds maximum size {bank_code_length}")

        if len(branch_code) > branch_code_length:
            raise exceptions.InvalidBranchCode(
                "Branch code exceeds maximum size {branch_code_length}"
            )

        if len(account_code) > account_code_length:
            raise exceptions.InvalidAccountCode(
                "Account code exceeds maximum size {account_code_length}"
            )
    ...
    def _validate_characters(self):
        if not re.match(r"[A-Z]{2}\d{2}[A-Z]*", self.compact):
            raise exceptions.InvalidStructure("Invalid characters in IBAN {self.compact}")
